...
def get_inp_args():
    ...
    parser.add_argument('--cat_name_pth', type=str, default='cat_to_name.json', 
                       help='path to json file containing class numbers and corresponding names')
    parser.add_argument('--device', type=str, default='cuda', choices=['cuda', 'cpu'], 
                        help='device on which to predict, either cuda gpu or cpu')
    parser.add_argument('--topk', type=int, default=5, help='number of top classes and probabilities to predict')
...
def predict(image_path, checkpoint_path, device, topk, cat_name_pth):
    ...
    with torch.no_grad():
        ...
        probs, class_indices = F.softmax(output.data,dim=1).topk(topk)
        ...
        cat_to_name = read_json(cat_name_pth)
        class_names = [cat_to_name[i] for i in classes]
    
    return classes, class_names, probs

inp = get_inp_args()

classes, class_names, probs = predict(inp.image_path, inp.chpt_path, inp.device, 4, inp.cat_name_pth)

print()
print("Classes:", classes,'|', 'Class names:', class_names,'|', 'Probabilities:', probs)
